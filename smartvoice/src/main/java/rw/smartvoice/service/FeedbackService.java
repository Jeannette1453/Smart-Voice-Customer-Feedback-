                ...
                if (ai.suggestedDepartment != null && !ai.suggestedDepartment.isBlank()) {
                    departmentRepository.findByNameIgnoreCase(ai.suggestedDepartment.trim())
                            .ifPresent(dep -> {
                                ...
                                // Auto-assign to least-busy STAFF in the same department
                                try {
                                    List<User> staffList = userRepository.findByRoleIn(
                                            List.of(Role.STAFF))
                                        .stream()
                                        .filter(s -> s.getDepartment() != null &&
                                                     s.getDepartment().getId().equals(dep.getId()))
                                        .collect(java.util.stream.Collectors.toList());

                                    // Fall back to any STAFF if none in this department
                                    if (staffList.isEmpty()) {
                                        staffList = userRepository.findByRoleIn(List.of(Role.STAFF));
                                    }
                                    if (!staffList.isEmpty()) {
                                        // Build workload map
                                        java.util.Map<UUID, Long> workload = new java.util.HashMap<>();
                                        for (Object[] row : feedbackRepository.countOpenCasesByStaff()) {
                                            workload.put((UUID) row[0], ((Number) row[1]).longValue());
                                        }
                                        // Pick staff with fewest open cases
                                        User leastBusy = staffList.stream()
                                                .min(java.util.Comparator.comparingLong(
                                                        s -> workload.getOrDefault(s.getId(), 0L)))
                                                .orElse(null);
                                        if (leastBusy != null) {
                                            f.setAssignedStaff(leastBusy);
                                            // Notify the auto-assigned staff by email + notification
                                            final User assignedStaff = leastBusy;
                                            final String deptName = dep.getName();
                                            try {
                                                emailService.sendSimpleEmail(
                                                    assignedStaff.getEmail(),
                                                    "SmartVoice: New Feedback Auto-Assigned To You",
                                                    "Hello " + assignedStaff.getFullName() + ",\n\n"
                                                        + "A new feedback has been automatically assigned to you.\n\n"
                                                ...
                                                notificationService.createNotification(
                                                    ...
                                                    "A feedback from " + customer.getFullName()
                                                        + " about \"" + (req.category != null ? req.category : "General") + "\" has been assigned to you.",
                                                    customer.getFullName(),
                                                    customer.getEmail()
                                                );
                                            } catch (Exception ex) {
                                                System.out.println("AUTO-ASSIGN STAFF EMAIL ERROR => " + ex.getMessage());
                                            }
                                        }
                                    }
                                } catch (Exception ex) {
                                    System.out.println("AUTO-ASSIGN STAFF ERROR => " + ex.getMessage());
                                }
                            });
                }
            }
